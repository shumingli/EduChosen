...
+ (void)openAppStoreWithAppId:(NSString *)appId withViewControler:(UIViewController *)delegate{
    SKStoreProductViewController *storeProductVC = [[SKStoreProductViewController alloc] init];
    storeProductVC.delegate =(id<SKStoreProductViewControllerDelegate>)delegate;
    
    NSDictionary *dict = [NSDictionary dictionaryWithObject:appId forKey:SKStoreProductParameterITunesItemIdentifier];
    [storeProductVC loadProductWithParameters:dict completionBlock:^(BOOL result, NSError *error){
        if (result){
            [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
            [delegate presentViewController:storeProductVC animated:YES completion:nil];
        }
    }];
}
